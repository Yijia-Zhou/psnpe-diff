...
import com.qualcomm.qti.psnpedemo.post.PostDetectionMobileNetV1SSD;
import com.qualcomm.qti.psnpedemo.post.PostDetectionYolov3;
...
public class DetectionPostprocessor extends PostProcessor {
    ...
    private PostDetectionYolov3 postDetectionYolov3;
    private PostDetectionMobileNetV1SSD postDetectionMobileNetV1SSD;
    ...
    public DetectionPostprocessor(EvaluationCallBacks evaluationCallBacks, ModelInfo modelInfo, int imageNumber) {
        ...
        postDetectionYolov3 = new PostDetectionYolov3();
        postDetectionMobileNetV1SSD = new PostDetectionMobileNetV1SSD();
    }
    @Override
...
//            return postDetectionYolov3.postProcessResult(inputImages, batchSize);
            int max = 50;
            ...
            for (int i = 0; i < executeTimes; i++){
                ...
                for (int j = 0; j < handleSize; j++){
                    ...
                    String imageName=inputImages.get(nowCount).getName();
                    String imageId=imageName.split("\\.")[0];
                    Id.add(imageId);
                    nowCount++;
                }
                postDetectionYolov3.postProcessResult(outputs, Id);
                outputs.clear();
                Id.clear();
            }
            return true;
        }
        if (modelName.contains("ssd")){
            return postDetectionMobileNetV1SSD.postProcessResult(inputImages, batchSize);
        }
    ...
    public void setResult(Result result) {
        if (modelName.contains("yolo")){
            postDetectionYolov3.saveData();
        }
        if (modelName.contains("ssd")){
            postDetectionMobileNetV1SSD.saveData();
        }
